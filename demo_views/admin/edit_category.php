...
$edit_info = getEditInfo($conn,$_GET['id'],'category');
$error= [];
if(array_key_exists('submit', $_POST)){
  if(empty($_POST['category'])){
    $error['category']="Enter a category name";
  }

  if(empty($error)){
    $clean = array_map('trim', $_POST);
    // editCampusArticle($conn, $clean,$_GET['id']);
    editCategory($conn, $clean,$_GET['id']);
  }
}
?>
<section id="content">
    ...
    <div class="row">
      ...
      <div class="col-sm-12 col-md-10 col-md-offset-1">
        <div class="page-ads box">
          <h2 class="title-2">Welcome to the Edit Category page</h2>
          ...
          <form class="form-ad" action="" method="post" enctype="multipart/form-data">
            <div class="form-group mb30">
              <label class="control-label">Category</label><?php $display = displayErrors($error, 'category');
              echo $display ?> <input class="form-control input-md" name="category" placeholder="Write a category name" value="<?php echo $edit_info['category_name']      ?>"  type="text">
            </div>
              </div>
              <br/>
